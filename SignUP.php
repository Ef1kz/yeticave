...
require 'data.php';
$con = mysqli_connect('127.0.0.1', 'root', '','schema3');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['user_avatar']) and !empty($_FILES['user_avatar']['name'])) {
        ...
        if ($file_type !== 'image/gif') {
            move_uploaded_file($_FILES['user_avatar']['tmp_name'], $file_path . $file_name);
        }
    ...
    if(count($errors!==0)){
        $user = [
            ...
            'user_avatar'=>$img_user ? 'img/user.jpg' : ''
        ];

        $page_content = compile_template('SignUp.php',
            [
                'errors' => $errors,
        ...
        VALUE ('{$user['name']}', '{$user['email']}', '{$user['password']}', '2021.03.26',   '{$user['user_avatar']}' '{$user['message']}')";
